...
def _sample_post_trajs_scaled(df_window: pd.DataFrame, df_basis: int = 5,
    n_samples: int = 1000, tune: int = 1000, min_points: int = 5,
    ...
    # prediction grid: monthly over the window (24 points if span≈2y)
    n_grid = max(12, int(round(span * 12)))
...
def _flags_from_traj(traj, time_grid, flat_thr=-1, decline_thr=-2, nonlinear_gap=3):
...
def _posterior_feature_probs_from_samples(y_samples, time_grid, flat_thr=-1, decline_thr=-2, nonlinear_gap=3):
    counts = {'prolonged_nonprogression': 0, 'linear_decline': 0, 'nonlinear': 0}
    S = y_samples.shape[0]
    for s in range(S):
        flags = _flags_from_traj(y_samples[s], time_grid, flat_thr, decline_thr, nonlinear_gap)
        for k in counts:
            counts[k] += int(flags[k])
    return {f"traj_prob_{k}": counts[k] / S for k in counts}


def _window_worker(window_df, flat_thr=-1, decline_thr=-2, nonlinear_gap=3, df_basis=5, n_samples=1000, tune=1000):
    """Compute posterior feature probabilities for a single window."""
    time_grid, y_samples = _sample_post_trajs_scaled(window_df, df_basis=df_basis, n_samples=n_samples, tune=tune)
    return _posterior_feature_probs_from_samples(y_samples, time_grid, flat_thr, decline_thr, nonlinear_gap)
...
def compute_time_varying_trajectory_covariates_parallel(
    ...
    results = Parallel(n_jobs=n_jobs, backend="loky", verbose=verbose)(
        delayed(_window_worker)(win_df, flat_thr=flat_thr, decline_thr=decline_thr, nonlinear_gap=nonlinear_gap,
                                df_basis=df_basis, n_samples=n_samples, tune=tune)
