...
def compute_biomarker_trajectories(biomarker_name, config_dict, data_dir, window_hours, n_bootstrap, n_batches, cohort_patients=None):
    print("\n" + "=" * 80)
    ...
    if id_col not in ts_df.columns:
        for candidate in ['hadm_id', 'stay_id', 'patientid']:
    ...
    n_patients = ts_df[id_col].nunique()
    n_rows = len(ts_df)
    ...
    print(f"   Admissions: {n_patients:,}")
    print(f"   Measurements: {n_rows:,}")
    print(f"   Measurements/admission: {n_rows / n_patients:.1f}")
    ...
    # Detect time columns - MIMIC may use time_days or time_hours
    ...
    elif 'time_days' in ts_df.columns:
        # Create time_hour from time_days
        ts_df['time_hours'] = ts_df['time_days'] * 24.0
        ts_df['time_hour'] = np.floor(ts_df['time_hours']).astype(int)
        time_col, windowing_col = 'time_hours', 'time_hour'
    else:
        print(f"   ⚠️  WARNING: No time columns found, skipping {biomarker_name}")
        return None

    id_time_map = ts_df[[id_col, windowing_col]].drop_duplicates()

    traj_input = ts_df[[id_col, time_col, windowing_col, value_col]].copy()
    ...
    if cohort_patients is not None:
        ...
        print(f"   Filtered to cohort: {traj_input['patientid'].nunique():,} admissions")
    ...
    for i in range(n_batches):
        ...
        print(f"   Batch {i + 1}/{n_batches} ({len(subset_patients)} admissions)...", end=' ', flush=True)
    ...
    trajectory_probs = trajectory_probs.rename(columns={'patientid': id_col})
    ...
    if windowing_col not in trajectory_probs.columns:
        ...
        trajectory_probs = trajectory_probs.merge(id_time_map, on=id_col, how='left', suffixes=('', '_reconstructed'))
    ...
    result_cols = [id_col, windowing_col, f'{biomarker_name}_stable', f'{biomarker_name}_gradual', f'{biomarker_name}_rapid']
    ...
    return result, id_col, windowing_col


def main():
    parser = argparse.ArgumentParser(description='Compute MIMIC bootstrap circulatory failure trajectory probabilities')
    parser.add_argument('--data-dir', type=str, default='/home/gaga/data/physionet/mimic/circulatory_failure',
                        help='Directory containing biomarker time series CSVs')
    parser.add_argument('--pred-dataset', type=str, default='/home/gaga/data/physionet/mimic/circulatory_failure/circulatory_failure_prediction_dataset.csv',
    ...
    biomarker_results = {}
    detected_id_col = 'hadm_id'
    detected_windowing_col = 'time_hour'
    ...
    if args.cohort_splits > 1:
        if args.pred_dataset and Path(args.pred_dataset).exists():
            ...
            id_col = 'hadm_id' if 'hadm_id' in prediction_dataset.columns else 'stay_id'
            ...
            if ts_path.exists():
                ts_df = pd.read_csv(ts_path)
                id_col = 'hadm_id' if 'hadm_id' in ts_df.columns else 'stay_id'
        ...
        if all_patients is not None:
            ...
            print(f"   Processing admissions {start_idx:,} to {end_idx:,} (of {total_patients:,} total)")
            print(f"   Cohort size: {len(cohort_patients):,} admissions")
    ...
    for biomarker, cfg in biomarkers_to_process.items():
        ...
        if result is not None:
            traj_df, detected_id_col, detected_windowing_col = result
    ...
    if args.pred_dataset and Path(args.pred_dataset).exists():
        ...
        windowing_col = detected_windowing_col
