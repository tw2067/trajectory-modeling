#!/bin/bash
# Launch Mimic Aki Analysis via nohup
# Usage: ./scripts/launchers/mimic/run_mimic_aki_nohup.sh [--train-n-jobs N] [--train-backend threading|processes] [--parallel-axis repeat|feature-set] [--save-oof|--no-save-oof]
...
echo "Starting Mimic Aki Analysis (nohup)"
...
nohup python scripts/analysis/mimic/mimic_aki_analysis.py \
    ...
    "${BOOTSTRAP_ARGS[@]}" \
    "${OOF_ARGS[@]}" \
    > logs/outs/mimic/mimic_aki_analysis.out 2> logs/errs/mimic/mimic_aki_analysis.err &

PID=$!
echo "Process started with PID: $PID"
echo $PID > .mimic_aki_analysis.pid

echo "Output: logs/outs/mimic/mimic_aki_analysis.out"
echo "Errors: logs/errs/mimic/mimic_aki_analysis.err"
echo "To monitor: tail -f logs/outs/mimic/mimic_aki_analysis.out"
