...
# Usage: ./scripts/run_mimic_sepsis_nohup.sh [--train-n-jobs N] [--train-backend threading|processes] [--parallel-axis repeat|feature-set] [--save-oof|--no-save-oof]
...
nohup python scripts/analysis/mimic/mimic_sepsis_analysis.py \
    ...
    --plot-k "$PLOT_K" \
    "${OOF_ARGS[@]}" \
    > logs/outs/mimic_sepsis_analysis.out 2>&1 &

PID=$!
echo "Process started with PID: $PID"
echo $PID > .mimic_sepsis_analysis.pid

echo "Output: logs/outs/mimic_sepsis_analysis.out"
echo "To monitor: tail -f logs/outs/mimic_sepsis_analysis.out"
