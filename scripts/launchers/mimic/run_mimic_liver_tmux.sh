#!/bin/bash
# Launch Mimic Liver Analysis via tmux
# Usage: ./scripts/launchers/mimic/run_mimic_liver_tmux.sh [--train-n-jobs N] [--train-backend threading|processes] [--parallel-axis repeat|feature-set] [--save-oof|--no-save-oof]

REPO_ROOT="${REPO_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)}"

SESSION_NAME="mimic_liver_analysis"
...
tmux send-keys -t "$SESSION_NAME" \
    "cd $REPO_ROOT && python scripts/analysis/mimic/mimic_liver_analysis.py --train-n-jobs $TRAIN_N_JOBS --train-backend $TRAIN_BACKEND --parallel-axis $PARALLEL_AXIS --plot-k $PLOT_K --with-probs-source $WITH_PROBS_SOURCE $BOOTSTRAP_FLAG --oof-scope $OOF_SCOPE --oof-k $OOF_K $OOF_FLAG > logs/outs/mimic/mimic_liver_analysis.out 2> logs/errs/mimic/mimic_liver_analysis.err" \
    Enter
...
echo "  Output log: logs/outs/mimic/mimic_liver_analysis.out"
echo "  Error log: logs/errs/mimic/mimic_liver_analysis.err"
echo ""
