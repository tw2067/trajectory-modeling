...
# Usage: ./scripts/run_mimic_ventilator_nohup.sh [--train-n-jobs N] [--train-backend threading|processes] [--parallel-axis repeat|feature-set] [--save-oof|--no-save-oof]
...
nohup python scripts/analysis/mimic/mimic_ventilator_analysis.py \
    ...
    --plot-k "$PLOT_K" \
    "${OOF_ARGS[@]}" \
    > logs/outs/mimic_ventilator_analysis.out 2>&1 &

PID=$!
echo "Process started with PID: $PID"
echo $PID > .mimic_ventilator_analysis.pid

echo "Output: logs/outs/mimic_ventilator_analysis.out"
echo "To monitor: tail -f logs/outs/mimic_ventilator_analysis.out"
