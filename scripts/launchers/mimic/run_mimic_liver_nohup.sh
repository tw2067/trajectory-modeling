...
# Usage: ./scripts/launchers/mimic/run_mimic_liver_nohup.sh [--train-n-jobs N] [--train-backend threading|processes] [--parallel-axis repeat|feature-set] [--save-oof|--no-save-oof]
...
nohup python scripts/analysis/mimic/mimic_liver_analysis.py \
    ...
    "${OOF_ARGS[@]}" \
    > logs/outs/mimic/mimic_liver_analysis.out 2> logs/errs/mimic/mimic_liver_analysis.err &

PID=$!
echo "Process started with PID: $PID"
echo $PID > .mimic_liver_analysis.pid

echo "Output: logs/outs/mimic/mimic_liver_analysis.out"
echo "Errors: logs/errs/mimic/mimic_liver_analysis.err"
echo "To monitor: tail -f logs/outs/mimic/mimic_liver_analysis.out"
