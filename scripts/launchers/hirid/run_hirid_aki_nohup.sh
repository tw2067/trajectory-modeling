...
# Usage: ./scripts/launchers/hirid/run_hirid_aki_nohup.sh [--train-n-jobs N] [--train-backend threading|processes] [--parallel-axis repeat|feature-set] [--save-oof|--no-save-oof]
...
nohup python scripts/analysis/hirid/hirid_aki_analysis.py \
    ...
    "${OOF_ARGS[@]}" \
    > logs/outs/hirid/hirid_aki_analysis.out 2> logs/errs/hirid/hirid_aki_analysis.err &

PID=$!
echo "Process started with PID: $PID"
echo $PID > .hirid_aki_analysis.pid

echo "Output: logs/outs/hirid/hirid_aki_analysis.out"
echo "Errors: logs/errs/hirid/hirid_aki_analysis.err"
echo "To monitor: tail -f logs/outs/hirid/hirid_aki_analysis.out"
