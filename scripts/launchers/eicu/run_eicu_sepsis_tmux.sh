#!/bin/bash
# Launch Eicu Sepsis Analysis via tmux
# Usage: ./scripts/run_eicu_sepsis_tmux.sh [--train-n-jobs N] [--train-backend threading|processes] [--parallel-axis repeat|feature-set] [--save-oof|--no-save-oof]

SESSION_NAME="eicu_sepsis_analysis"
...
tmux send-keys -t "$SESSION_NAME" \
    "cd /home/gaga/tamarw1/trajectory-modeling && python scripts/analysis/eicu/eicu_sepsis_analysis.py --train-n-jobs $TRAIN_N_JOBS --train-backend $TRAIN_BACKEND --lookback-window $LOOKBACK_WINDOW --lookback-unit $LOOKBACK_UNIT --parallel-axis $PARALLEL_AXIS --plot-k $PLOT_K --oof-scope $OOF_SCOPE --oof-k $OOF_K $OOF_FLAG" \
    Enter
