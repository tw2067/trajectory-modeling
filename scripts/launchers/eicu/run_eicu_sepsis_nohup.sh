#!/bin/bash
# Launch Eicu Sepsis Analysis via nohup
# Usage: ./scripts/launchers/eicu/run_eicu_sepsis_nohup.sh [--train-n-jobs N] [--train-backend threading|processes] [--parallel-axis repeat|feature-set] [--save-oof|--no-save-oof]
...
echo "Starting Eicu Sepsis Analysis (nohup)"
...
nohup python scripts/analysis/eicu/eicu_sepsis_analysis.py \
    ...
    "${BOOTSTRAP_ARGS[@]}" \
    "${OOF_ARGS[@]}" \
    > logs/outs/eicu/eicu_sepsis_analysis.out 2> logs/errs/eicu/eicu_sepsis_analysis.err &

PID=$!
echo "Process started with PID: $PID"
echo $PID > .eicu_sepsis_analysis.pid

echo "Output: logs/outs/eicu/eicu_sepsis_analysis.out"
echo "Errors: logs/errs/eicu/eicu_sepsis_analysis.err"
echo "To monitor: tail -f logs/outs/eicu/eicu_sepsis_analysis.out"
