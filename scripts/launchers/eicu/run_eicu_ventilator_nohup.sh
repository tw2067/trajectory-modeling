...
# Usage: ./scripts/launchers/eicu/run_eicu_ventilator_nohup.sh [--train-n-jobs N] [--train-backend threading|processes] [--parallel-axis repeat|feature-set] [--save-oof|--no-save-oof]
...
nohup python scripts/analysis/eicu/eicu_ventilator_analysis.py \
    ...
    "${OOF_ARGS[@]}" \
    > logs/outs/eicu/eicu_ventilator_analysis.out 2> logs/errs/eicu/eicu_ventilator_analysis.err &

PID=$!
echo "Process started with PID: $PID"
echo $PID > .eicu_ventilator_analysis.pid

echo "Output: logs/outs/eicu/eicu_ventilator_analysis.out"
echo "Errors: logs/errs/eicu/eicu_ventilator_analysis.err"
echo "To monitor: tail -f logs/outs/eicu/eicu_ventilator_analysis.out"
