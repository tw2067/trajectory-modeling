#!/bin/bash
# Launch Eicu Liver Analysis via nohup
# Usage: ./scripts/launchers/eicu/run_eicu_liver_nohup.sh [--train-n-jobs N] [--train-backend threading|processes] [--parallel-axis repeat|feature-set] [--save-oof|--no-save-oof]
...
echo "Starting Eicu Liver Analysis (nohup)"
...
nohup python scripts/analysis/eicu/eicu_liver_analysis.py \
    ...
    "${OOF_ARGS[@]}" \
    > logs/outs/eicu/eicu_liver_analysis.out 2> logs/errs/eicu/eicu_liver_analysis.err &

PID=$!
echo "Process started with PID: $PID"
echo $PID > .eicu_liver_analysis.pid

echo "Output: logs/outs/eicu/eicu_liver_analysis.out"
echo "Errors: logs/errs/eicu/eicu_liver_analysis.err"
echo "To monitor: tail -f logs/outs/eicu/eicu_liver_analysis.out"
