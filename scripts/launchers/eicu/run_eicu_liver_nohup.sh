...
# Usage: ./scripts/run_eicu_liver_nohup.sh [--train-n-jobs N] [--train-backend threading|processes] [--parallel-axis repeat|feature-set] [--save-oof|--no-save-oof]
...
nohup python scripts/analysis/eicu/eicu_liver_analysis.py \
    ...
    --plot-k "$PLOT_K" \
    "${OOF_ARGS[@]}" \
    > logs/outs/eicu_liver_analysis.out 2>&1 &

PID=$!
echo "Process started with PID: $PID"
echo $PID > .eicu_liver_analysis.pid

echo "Output: logs/outs/eicu_liver_analysis.out"
echo "To monitor: tail -f logs/outs/eicu_liver_analysis.out"
