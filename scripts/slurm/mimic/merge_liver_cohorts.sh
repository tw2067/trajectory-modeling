#!/usr/bin/env bash
# Merge all MIMIC liver trajectory sub-cohort files into single datasets
...
echo "Merging MIMIC liver trajectory probability sub-cohorts..."
...
result_dir = Path(os.environ.get("DATA_ROOT", "/home/gaga/data/physionet")) / "mimic" / "liver"
...
merge_cohorts("bili_trajectory_probs_bayes_cohort*.csv", "bili_trajectory_probs_bayes.csv", "hadm_id")
merge_cohorts("liver_prediction_dataset_with_probs_cohort*.csv", "liver_prediction_dataset_with_probs.csv", "hadm_id")
EOF
