"""
Compute P/F ratio trajectory probabilities for eICU ventilator cohort.

Usage:
    python ventilator_trajs.py --input results/eicu/ventilator/pf_ratio_timeseries.csv --pred-dataset results/eicu/ventilator/ventilator_prediction_dataset.csv --output results/eicu/ventilator/ventilator_trajectory_probs.csv
"""
...
def main():
    ...
    parser.add_argument('--input', type=str,
                       default=os.path.join(_DATA_ROOT, 'eicu', 'ventilator', 'pf_ratio_timeseries.csv'),
                       help='Path to raw P/F ratio time series CSV')
    parser.add_argument('--pred-dataset', type=str,
                       default=os.path.join(_DATA_ROOT, 'eicu', 'ventilator', 'ventilator_prediction_dataset.csv'),
                       help='Path to prediction dataset for merging')
    parser.add_argument('--output', type=str,
                       default=os.path.join(_DATA_ROOT, 'eicu', 'ventilator', 'ventilator_trajectory_probs.csv'),
                       help='Path to save prediction dataset merged with probabilities')
    parser.add_argument('--probs-output', type=str,
                       default=os.path.join(_DATA_ROOT, 'eicu', 'ventilator', 'ventilator_trajectory_probs_bayes.csv'),
    ...
    parser.add_argument('--flat-thr', type=float, default=10.0,
                       help='Stable threshold for P/F ratio (default: 10.0)')
    parser.add_argument('--decline-thr', type=float, default=30.0,
                       help='Improvement threshold for P/F ratio per day (default: 30.0)')
    parser.add_argument('--nonlinear-gap', type=float, default=20.0,
                       help='Nonlinear gap threshold (default: 20.0)')
    ...
    print("="*80)
    print("eICU Ventilator Trajectory Modeling")
    if args.cohort_splits > 1:
        print(f"Sub-cohort: {args.cohort_index + 1}/{args.cohort_splits}")
    print("="*80)
    
    # Load raw P/F ratio time series
    print(f"\n📊 Loading raw P/F ratio time series from: {args.input}")
    pf_ts = pd.read_csv(args.input)
    
    print(f"   Patients: {pf_ts['stay_id'].nunique():,}")
    print(f"   Total measurements: {len(pf_ts):,}")
    print(f"   Measurements per patient: {len(pf_ts) / pf_ts['stay_id'].nunique():.1f}")
    ...
    # Note: For P/F ratio, INCREASE is good (better oxygenation), so we invert the interpretation
    config = BayesConfig(
    ...
    print(f"   Stable threshold: ±{args.flat_thr}")
    print(f"   Change threshold: >{args.decline_thr} per day")
    ...
    # Rename columns - for P/F, "decline" means IMPROVING (higher P/F is better)
    # So we keep the terminology but interpret it as improvement
    trajectory_probs = trajectory_probs.rename(columns={
        'trajtype_prolonged_nonprogression_prob': 'prob_stable',
        'trajtype_linear_decline_prob': 'prob_gradual_improvement',  # Higher P/F = better
        'trajtype_nonlinear_prob': 'prob_rapid_improvement',         # Rapid P/F increase = better
        'patientid': 'stay_id'
    })
