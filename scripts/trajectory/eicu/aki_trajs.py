"""
Compute creatinine trajectory probabilities for eICU AKI cohort.

Usage:
    python aki_trajs.py --input results/eicu/aki/creatinine_timeseries.csv --pred-dataset results/eicu/aki/aki_prediction_dataset.csv --output results/eicu/aki/aki_trajectory_probs.csv
"""
...
os.environ['FILELOCK_TIMEOUT'] = '30'  # Increase from default 10s
...
def main():
    ...
    parser.add_argument('--n-batches', type=int, default=20,
                       help='Number of batches for processing (default: 20, increased to reduce parallelism per batch)')
    
    # Sub-cohort splitting arguments
    parser.add_argument('--cohort-splits', type=int, default=1,
                       help='Total number of cohort splits (default: 1 = no splitting)')
    parser.add_argument('--cohort-index', type=int, default=0,
                       help='Which cohort split to process (0-indexed, default: 0)')
    
    args = parser.parse_args()
    
    # Validate cohort split arguments
    ...
    print(f"   Total measurements: {len(creatinine_ts):,}")
    print(f"   Measurements per patient: {len(creatinine_ts) / creatinine_ts['stay_id'].nunique():.1f}")
    
    # Prepare trajectory input (rename for BayesianTraj)
    traj_input = creatinine_ts[['stay_id', 'time_days', 'time_day', 'creatinine']].copy()
    ...
    # Apply sub-cohort splitting if requested
    all_patients = traj_input['patientid'].unique()
    total_patients = len(all_patients)
    
    if args.cohort_splits > 1:
        # Split patients into cohorts
        cohort_size = total_patients // args.cohort_splits
        start_idx = args.cohort_index * cohort_size
        if args.cohort_index == args.cohort_splits - 1:
            # Last cohort gets remaining patients
    ...
    for i in range(n_batches):
        ...
        # Clear PyTensor cache after each batch to prevent lock contention accumulation
        gc.collect()
        if PYTENSOR_CACHE.exists():
            try:
                print(f"   Cleaning PyTensor cache for next batch...")
