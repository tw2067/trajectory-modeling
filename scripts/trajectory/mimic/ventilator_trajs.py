"""
Compute P/F ratio trajectory probabilities for MIMIC ventilator cohort.

Usage:
    python ventilator_trajs.py --input results/mimic/ventilator/pf_ratio_timeseries.csv --pred-dataset results/mimic/ventilator/ventilator_prediction_dataset.csv --output results/mimic/ventilator/pf_trajectory_probs_bayes.csv
"""
...
def main():
    ...
    parser.add_argument('--input', type=str,
                        default=os.path.join(_DATA_ROOT, 'mimic', 'ventilator', 'pf_ratio_timeseries.csv'),
                        help='Path to raw P/F ratio time series CSV')
    parser.add_argument('--pred-dataset', type=str,
                        default=os.path.join(_DATA_ROOT, 'mimic', 'ventilator', 'ventilator_prediction_dataset.csv'),
                        help='Path to prediction dataset for merging')
    parser.add_argument('--output', type=str,
                        default=os.path.join(_DATA_ROOT, 'mimic', 'ventilator', 'pf_trajectory_probs_bayes.csv'),
    ...
    print("=" * 80)
    print("MIMIC Ventilator Trajectory Modeling")
    ...
    n_patients = pf_ts['hadm_id'].nunique()
    n_rows = len(pf_ts)
    ...
    trajectory_probs = trajectory_probs.rename(columns={
        ...
        'trajtype_linear_decline_prob': 'prob_gradual_improvement',
    ...
    if args.pred_dataset and Path(args.pred_dataset).exists():
        ...
        prediction_dataset = prediction_dataset[prediction_dataset['hadm_id'].isin(traj_input['patientid'].unique())]
        ...
        merged_path = Path(args.merged_output) if args.merged_output else output_path.with_name('ventilator_prediction_dataset_with_probs.csv')
        if args.cohort_splits > 1:
