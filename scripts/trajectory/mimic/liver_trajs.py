"""
Compute bilirubin trajectory probabilities for MIMIC liver cohort.

Usage:
    python liver_trajs.py --input results/mimic/liver/bilirubin_timeseries.csv --pred-dataset results/mimic/liver/liver_prediction_dataset.csv --output results/mimic/liver/bili_trajectory_probs_bayes.csv
"""
...
def main():
    ...
    print("=" * 80)
    print("MIMIC Liver Trajectory Modeling")
    ...
    n_patients = bilirubin_ts['hadm_id'].nunique()
    n_rows = len(bilirubin_ts)
    ...
    traj_input = bilirubin_ts[['hadm_id', 'time_days', 'time_day', 'bilirubin']].copy()
    ...
    if args.pred_dataset and Path(args.pred_dataset).exists():
        ...
        merged_path = Path(args.merged_output) if args.merged_output else output_path.with_name('liver_prediction_dataset_with_probs.csv')
        if args.cohort_splits > 1:
