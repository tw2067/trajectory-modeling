...
def compute_biomarker(
    ...
    print("=" * 80)
    print(f"MIMIC Circulatory Failure Trajectory Modeling - {biomarker.upper()}")
    print("=" * 80)
    ...
    traj_input = ts_df[['hadm_id', 'time_hours', 'time_hour', value_col]].copy()
    ...
    print(f"   Timepoints per patient: {traj_input.groupby('patientid')['time_hours'].count().mean():.1f}")
...
def main():
    parser = argparse.ArgumentParser(description='Compute MIMIC circulatory failure trajectory probabilities')
    ...
    if args.biomarker == 'all':
        ...
        for biomarker in BIOMARKER_CONFIG.keys():
            ...
            prefixed = traj_probs[['hadm_id', 'time_hour'] + prob_cols].copy()
            rename_map = {
        ...
        if args.pred_dataset and Path(args.pred_dataset).exists():
            ...
            for biomarker, traj_df in biomarker_results.items():
                ...
                merged = merged.merge(traj_df, on=['hadm_id', 'time_hour'], how='left')
    ...
    if args.pred_dataset and Path(args.pred_dataset).exists():
        ...
        merge_cols = ['hadm_id', 'time_hour'] + prob_cols
